...
router.put('/:id', async (req, res) => {
  try {
    ...
    const macRegex = /^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/;
    if (!macRegex.test(mac_address)) {
      return res.status(400).json({ error: 'Please enter a valid MAC address' });
    }

    const formattedMac = mac_address.toUpperCase().replace(/-/g, ':');

    const updatedMac = await DeviceMacAddress.findByIdAndUpdate(
      req.params.id,
      { device_id, mac_address: formattedMac },
      { new: true }
    ).populate('device_id', 'name location');

    if (!updatedMac) return res.status(404).json({ error: 'MAC address not found' });

    res.json(updatedMac);
  } catch (error) {
    if (error.code === 11000) {
      return res.status(400).json({ error: 'This MAC address is already assigned' });
    }
    res.status(500).json({ error: error.message });
  }
});
